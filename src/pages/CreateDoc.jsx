...
const loadDocument = async (docId) => {
  ...
  try {
    const response = await fetch(`https://viadocs-backend-production.up.railway.app/api/docs/my-docs/${docId}`, {
  ...
  const delayDebounce = setTimeout(async () => {
    ...
    try {
      const res = await axios.post(
        "https://viadocs-backend-production.up.railway.app/api/docs/check-name",
...
const handleRename = async (e) => {
  ...
  if (!docId) {
    ...
    const res = await fetch("https://viadocs-backend-production.up.railway.app/api/docs/my-docs", {
      headers: { Authorization: `Bearer ${token}` },
    });
    ...
    const updateRes = await fetch(`https://viadocs-backend-production.up.railway.app/api/docs/my-docs/${docId}`, {
      method: "PUT",
      headers: {
  ...
  try {
    ...
    // Create new doc
    const response = await fetch("https://viadocs-backend-production.up.railway.app/api/docs/my-docs", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
  ...
  const toggleFavorite = async () => {
    ...
    try {
      const response = await fetch(`https://viadocs-backend-production.up.railway.app/api/docs/my-docs/${docId}/favorite`, {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${token}` }
      });
...
const deleteDocument = async () => {
  try {
    await axios.delete(`https://viadocs-backend-production.up.railway.app/api/docs/my-docs/${docId}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
...
const handleImageUpload = async (event) => {
  ...
  try {
    ...
    const response = await fetch("https://viadocs-backend-production.up.railway.app/api/docs/upload-image", {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
      },
    ...
    if (response.ok && data.url) {
      ...
      if (imgEl) {
        ...
        if (finalUrl.startsWith('/')) {
          const backendOrigin = window.location.origin.includes('localhost') ? 'https://viadocs-backend-production.up.railway.app' : window.location.origin;
