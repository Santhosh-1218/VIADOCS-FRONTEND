...
export default function Home() {
  ...
  const fetchDocs = useCallback(async () => {
    ...
    try {
      const res = await axios.get("https://viadocs-backend-production.up.railway.app/api/docs/my-docs", {
  ...
  useEffect(() => {
    const checkRole = async () => {
      if (!isLoggedIn) return;
      try {
        const res = await axios.get("https://viadocs-backend-production.up.railway.app/api/profile", {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (!res.data.role) {
          // open role modal
  ...
  const saveRole = async (role) => {
    ...
    try {
      ...
      const res = await axios.post(
        "https://viadocs-backend-production.up.railway.app/api/profile/role",
        { role },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const shareDocAsPDF = async (doc) => {
    try {
      const res = await axios.get(`https://viadocs-backend-production.up.railway.app/api/docs/my-docs/${doc._id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axios.delete(`https://viadocs-backend-production.up.railway.app/api/docs/my-docs/${deleteTarget._id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
